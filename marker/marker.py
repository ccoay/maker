...
def my_add_mark(imagePath, mark, out_path, quality):
    '''
    添加水印，然后保存图片
    '''
    im = Image.open(imagePath)
    ...
    if image:
        ...
        new_name = os.path.join(out_path, name)
        if os.path.splitext(new_name)[1] != '.png':
            image = image.convert('RGB')
...
def my_gen_mark(mark, size, color, opacity, space, angle):
    '''
    生成mark图片，返回添加水印的函数
    '''
    mark_str = mark
...
def produce_mark_end(mark, file_path, out_path='../output', size=25, color="#8B8B1B", opacity=0.15, space=40, angle=30, quality=80):
    if isinstance(mark, str) and sys.version_info[0] < 3:
        mark = mark.decode("utf-8")

    mark = my_gen_mark(mark, size, color, opacity, space, angle)

    if os.path.isdir(file_path):
        names = os.listdir(file_path)
        for name in names:
            image_file = os.path.join(file_path, name)
            my_add_mark(image_file, mark, out_path, quality)
    else:
        my_add_mark(file_path, mark, out_path, quality)
...
#     produce_mark_end('good', '../input', '../out/')
